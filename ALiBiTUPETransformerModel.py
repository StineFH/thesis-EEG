...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, PE_r, PE, mask=None):
        # Attention part
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, PE_r, PE=None, mask=None):
        for layer in self.layers:
            x = layer(x, PE_r, PE, mask=mask)
        return x

    def get_attention_maps(self, x, PE_r, PE=None, mask=None):
        attention_maps = []
        for idx, layer in enumerate(self.layers):
            print("Going into layer ", idx, "in TransformerEncoder")
            _, attn_map = layer.self_attn(x, PE_r, PE, mask=mask, return_attention=True)
