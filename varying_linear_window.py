...
def runCurrentModel(dl_train, dl_val, window):
    ######################## Make Neptune Logger ############################
    ...
    neptune_logger = pl.loggers.NeptuneLogger(
        ...
        source_files=["varying_linear_window.py", 
    ...
    neptune_logger.log_hyperparams({'beforePts':window, 'afterPts':window})
    ...
    torch.save(MAE, './linear_model_prediction_error/' + 'MAE_' + neptune_logger.version + '.pt')
...
if __name__ == '__main__':
    ...
    outputs = {} 
    context_window = [128, 256, 1024, 2048] # The one on 512 is THES-71
    
    for window in context_window:
        dl_train, dl_val = getData(sessionIds, channelIdxs, window, window, 
                                   targetPts, batchSize, train_size, limit_val)
        
        MAE_MSE = runCurrentModel(dl_train, dl_val, window)
        
        outputs[str(window)] = MAE_MSE
        print('Val loss for', window, ': ', outputs[str(window)])
